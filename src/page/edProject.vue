      ...
      <div class="reProject-body">
        <ul class="reProject-con">
          ...
          <li>
            ...
            <div class="reProject-address" @click="addGet" v-else-if="proAdd"><label>项目地址</label><span class="reProject-deAdd">{{proAdd}}</span></div>
            <div class="reProject-address" @click="addGet" v-else><label>项目地址</label><span class="reProject-deAdd">请选择项目地址</span></div>
          </li>
          <li class="reProject-budget">
            <label>预&nbsp;&nbsp;&nbsp;&nbsp;算</label>
            <input type="number" placeholder="填写项目预算" v-model="budget" v-if="proInfo.budget" readonly>
            <input type="number" placeholder="填写项目预算"  v-else>
            <span>万元</span>
          </li>
          <li>
            <label>面&nbsp;&nbsp;&nbsp;&nbsp;积</label>
            <input type="number" placeholder="填写项目面积" v-model="proSize" v-if="proInfo.proSize" readonly>
            <input type="number" placeholder="填写项目面积" v-else>
            <span>平方米</span>
          </li>
        </ul>
        <yd-cell-group>
      ...
      doContr(){
        ...
        $.ajax({
          ...
          success:(data)=> {
            ...
            $.each(h1val,(i,item)=>{
              let val=$(item).text();
              this.contractImg.push($.trim(val.substring(val.indexOf("MD5:") + 4)));
            })
...
//            this.contractImg.push(imgUrl);
            ...
            $.each(this.contractImg,(i,imgurl)=>{
      ...
      doConst(){
        ...
        $.ajax({
          ...
          success:(data)=> {
            ...
            $.each(h1val,(i,item)=>{
              let val=$(item).text();
              this.constructImg.push($.trim(val.substring(val.indexOf("MD5:") + 4)));
            })
//            let imgUrl=$.trim(data.substring(data.indexOf("MD5:") + 4, data.indexOf("</h1>")));
//            this.constructImg.push(imgUrl);
            ...
            $.each(this.constructImg,(i,imgurl)=>{
