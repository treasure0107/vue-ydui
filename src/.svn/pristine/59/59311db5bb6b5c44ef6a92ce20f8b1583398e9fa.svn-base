  ...
  <div class="aEdit">
    ...
    <div class="aEdit-body">
      <ul class="aEdit-record">
        <li @click="show" class="aEdit-item">增加记录</li>
        <li @click="show" class="aEdit-item">编辑记录</li>
        <li @click="show" class="aEdit-item">编辑是否公开浏览</li>
        <li @click="show" class="aEdit-item">删除记录</li>
      </ul>
    </div>
    ...
    <button class="reProject-send" @click="editProAuthorize">确定</button>
  </div>
</template>
<script>
  export default {
    ...
    created() {
      ...
      this.getProInfo();
      this.getUserProAuthorizeInfo();
    },
    methods: {
      getProInfo() {
        this.$http.post(this.$api.apihost+'project/getProInfo',{pid:this.$route.query.pid},{emulateJSON:true}).then   (function (response) {
          if(response.body.obj !=""){
            this.proInfo= response.body.obj;
          }
        });
      },
      getUserProAuthorizeInfo() {
          this.$http.post(this.$api.apihost+'user/getProAuthorizeInfo',{csrid:this.$route.query.csrid},{emulateJSON:true}).then   (function (response) {
          if(response.body.obj !=""){
            this.getProAuthorizeInfo= response.body.obj;
          }
        });
      },
      show(e){
      ...
      editProAuthorize(){
        ...
        this.$http.post(this.$api.apihost+'user/editProAuthorize',{csrid:csrid,suid:localStorage.suid,uid:localStorage.uid,addRecord,editRecord,editOpenStatus,delRecord,editAddr,editBudget,editProSize,editStartDate},{emulateJSON:true}).then(function(response){
          this.$router.go(-1);
        });
      },
