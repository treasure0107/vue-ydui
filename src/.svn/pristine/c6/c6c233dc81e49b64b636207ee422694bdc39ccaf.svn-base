        ...
        <yd-infinitescroll :on-infinite="loadList" ref="infinitescrollDemo">
          <yd-list theme="1" slot="list">
              <ul class="aCount-title">
                <li v-for="(item,index) in list" :key="index"><span class="aCount-name" @click="right(item.csrid,item.pid)">{{item.title}}</span><button class="aCount-cancel" @click="cancelAuthorize(item.suid,item.pid)">取消授权</button></li>
              </ul>
        </yd-list>
        <!-- 数据全部加载完毕显示 -->
