    ...
    <div class="reProject">
      ...
      <div class="proTitle" v-else-if="title">
        ...
        <div class="proTitle-show">
          <ul>
            <li @click="titleSel" :data-pid="item.pid" v-for="item in list">{{item.title}}</li>
          </ul>
        </div>
      </div>
      <div class="verCode" v-else-if="codeShow">
        <div class="verCode-tip"><h1>请提供您的手机号，以方便他人预约您的工地</h1></div>
      ...
      <div class="rePro" v-else>
        <div class="reProject-head">
          <textarea  v-model="content" placeholder="项目状态介绍（必填项）"></textarea>
          <ul class="reProject-headList">
            <li class="reProject-headItem" :class="index==0?'active':''"   v-for="(item,index) in this.picUrlval" :style="'background: url('+comimg+item+'?w=120&h=120) center center no-repeat;background-size:cover;'" :data-imgurl="item">
              <span class="reProject-headDel" @click="delRepic" :data-picUrl="item"></span>
              <div class="reProject-headCon" @click="changeCover">设为封面</div>
            </li>
            <li class="upload_img">
              <form id= "uploadForm">
                <label for="camear">+</label>
                <input type="file" accept="image/*" @change="doUpload" id="camear" name="file" multiple v-show="isiOS"/>
                <input type="file" accept="image/*" @change="doUpload" id="camear" name="file" capture="camera" v-show="isAndroid"/>
              </form>
            </li>
          </ul>
        ...
        <div class="reProject-body">
          <ul class="reProject-con">
            ...
            <li>
              <div class="reProject-address" v-if="getPro.address"><label>楼盘名称</label><span class="reProject-deAdd">{{getPro.address}}</span></div>
            ...
            <li class="reProject-budget">
              <label>预&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;算</label><input type="number" placeholder="填写项目预算" v-model="budget" v-if="getPro.budget" readonly><input type="number" placeholder="填写项目预算" v-model="budget" v-else>
              <span>万元</span>
            </li>
            <li>
              <label>面&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;积</label><input type="number" placeholder="填写项目面积" v-model="area"   v-if="getPro.proSize" readonly><input type="number" placeholder="填写项目面积" v-model="area" v-else>
              <span>平方米</span>
            </li>
          </ul>
          <yd-cell-group>
          ...
          <!--<ul class="reProject-footDeco" v-show="switch1">-->
            <!--<li>-->
              ...
              <!--<ul class="deco_contract">-->
                <!--<li v-for="vale in getPro.contractImgList" :style="'background: url('+comimg+vale.imgUrl+'?w=60) center center no-repeat;'"></li>-->
                <!--<li v-for="vale in contrImg" :style="'background: url('+comimg+vale+'?w=60) center center no-repeat;'" :data-imgurl="vale" @click="delContrPic"></li>-->
              <!--</ul>-->
              <!--<form id= "uploa">-->
                <!--<label for="came">上传</label>-->
                <!--<input type="file" accept="image/*" @change="doContr" id="came" name="file" multiple v-show="isiOS"/>-->
                <!--<input type="file" accept="image/*" @change="doContr" id="came" name="file" capture="camera" v-show="isAndroid"/>-->
              <!--</form>-->
            <!--</li>-->
            <!--<li>-->
              <!--<yd-checkbox v-model="checkbox2">提供施工图纸</yd-checkbox>-->
              <!--<ul class="deco_construct">-->
                <!--<li v-for="val in getPro.constructImgList" :style="'background: url('+comimg+val.imgUrl+'?w=60) center center no-repeat;'"></li>-->
                <!--<li v-for="val in constImg" :style="'background: url('+comimg+val+'?w=60) center center no-repeat;'"  :data-imgurl="val" @click="delConstPic"></li>-->
              <!--</ul>-->
              <!--<form id= "upload">-->
                <!--<label for="camea">上传</label>-->
                <!--<input type="file" accept="image/*" @change="doConst" id="camea" name="file" multiple v-show="isiOS"/>-->
                <!--<input type="file" accept="image/*" @change="doConst" id="camea" name="file" capture="camera" v-show="isAndroid"/>-->
              <!--</form>-->
            <!--</li>-->
          <!--</ul>-->
...
//        switch1: false,
        ...
        isAndroid: false,
        isiOS: false,
        titleBa:require('../images/space/project_selected.svg'),
...
//          alert("请输入验证码")
//        }else{
//          this.$http.post(this.$api.apihost+'user/editUserMobile',{uid:localStorage.uid,mobile:this.phone,code:this.code},{emulateJSON:true}).then(function (response) {
//            this.$router.go(-1);
//            this.codeShow=false;
//          });
//        }
...
//      项目标题点击
      titleSel(e){
        this.titleLis = $(e.target).text();
        if($(e.target).css("background-image")=="none"){
          $(e.target).css("background-image",'url('+this.titleBa+')');
        }else{
...
//            }
            ...
            if(data.position){
              this.positionGet=data.position;
            }
            if(data.address){
